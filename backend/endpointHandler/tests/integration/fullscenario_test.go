...
func TestFullScenarioFirst(t *testing.T) {
	...
	t.Run("GET /projects/{:id}", func(t *testing.T) {
		resp, err := client.Get(fmt.Sprintf("%s/projects/%s", baseURL, targetProject.Id))
		require.NoError(t, err)
		defer resp.Body.Close()
	...
	t.Run("GET /analytics/status-distribution", func(t *testing.T) {
		resp, err := client.Get(fmt.Sprintf("%s/analytics/status-distribution?key=%s", baseURL, targetProject.Key))
		require.NoError(t, err)
		defer resp.Body.Close()
...
func TestFullscenarioSecond(t *testing.T) {
	...
	// check that project in DB
	var loadProject []JiraProject
	var selectedLoaded []JiraProject
	t.Run("GET /projects", func(t *testing.T) {
		resp, err := client.Get(fmt.Sprintf("%s/projects", baseURL))
		require.NoError(t, err)
		defer resp.Body.Close()
		...
		err = json.NewDecoder(resp.Body).Decode(&loadProject)
		require.NoError(t, err)
		selectedLoaded = pickSyncedProjects(loadProject, p1, p2)
		require.GreaterOrEqual(t, len(selectedLoaded), 1)
	})

	// compare two projects
	t.Run("GET /compare/priority", func(t *testing.T) {
		if len(selectedLoaded) < 2 {
			t.Skip("not enough synced projects for compare endpoint")
		}
		url := fmt.Sprintf("%s/compare/priority?key=%s,%s", baseURL, selectedLoaded[0].Key, selectedLoaded[1].Key)
	...
	t.Run("DELETE /deleteProject", func(t *testing.T) {
		for _, prj := range selectedLoaded {
